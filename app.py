...
menu_option = st.sidebar.radio(
    "Select Page:",
    ["Home", "View Dataset", "Visualization", "Prediction","About"]
)
        ...
        # ================
            ...
            if missing_cols_ti:
                ...
                if compare_with_industry:
                    ...
                    # Layer them together (independent y-scales)
...
# ------------------
    ...
    st.header("Testing Algorithms")

    if df is None:
        st.error("Dataset could not be loaded. Please check the file path.")
    else:
        # Basic checks for needed columns
        required_cols_dm = ["Date", "Close", "Open", "High", "Low", "Volume", "Ticker"]
        missing_cols_dm = [c for c in required_cols_dm if c not in df.columns]
        if missing_cols_dm:
            st.error(f"The dataset must contain columns: {', '.join(missing_cols_dm)} for this demo.")
        else:
            st.markdown("""
            Select an algorithm from the dropdown below to predict whether 
            the **next day's Close** price will go **Up** or **Down** 
            based on today's **Close** and **Volume**.
            """)

            # 1. Create a menubar (selectbox) for algorithm selection
            algo_choice = st.selectbox(
                "Choose a Classification Algorithm:",
                ["Logistic Regression", "Decision Tree", "Random Forest"]
            )

            # 2. Let user select a Ticker
            all_tickers = df["Ticker"].unique().tolist()
            selected_ticker = st.selectbox("Select a Ticker:", all_tickers)

            # 3. Prepare data (common steps)
            data_algo = df[df["Ticker"] == selected_ticker].copy()
            data_algo["Date"] = pd.to_datetime(data_algo["Date"], errors='coerce')
            data_algo.dropna(subset=["Date"], inplace=True)
            data_algo.sort_values("Date", inplace=True)

            if len(data_algo) < 30:
                st.warning("Not enough data points for meaningful model training.")
            else:
                # Create Target: 1 if next day's close > today's close, else 0
                data_algo["Next_Close"] = data_algo["Close"].shift(-1)
                data_algo.dropna(subset=["Next_Close"], inplace=True)
                data_algo["Target"] = (data_algo["Next_Close"] > data_algo["Close"]).astype(int)

                # Features: Close, Volume
                X = data_algo[["Close", "Volume"]]
                y = data_algo["Target"]
                ...
                # Choose algorithm based on selectbox
                ...
                # 6. Optionally show next-day predictions
                if st.checkbox("Show next-day predictions for the test set"):
                    # Create a small dataframe with actual vs. predicted
                    results_df = X_test.copy()
                    results_df["Actual"] = y_test
                    results_df["Predicted"] = predictions
                    results_df["Prediction_Up/Down"] = results_df["Predicted"].replace({0: "Down", 1: "Up"})

                    st.write("Below are the last few rows of the test set with predictions:")
                    st.write(results_df.tail(10))

                st.markdown("""
                **Key:**
                - **Accuracy**: % of correct predictions (Up/Down).
                - **Precision**: Among predicted 'Up', how many were truly 'Up'?
                - **Recall**: Among actual 'Up' days, how many did we predict as 'Up'?
                """)
